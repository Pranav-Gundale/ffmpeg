...
def sweet_spot(video_kbps, orig_w, orig_h, fps):
    ...
    # max pixels where BPP = BPP_GOOD exactly
    max_pixels = (video_kbps * 1000) / (fps * BPP_GOOD)
    # aspect ratio: h = w * (orig_h / orig_w)
    # pixels = w * h = w * w * (orig_h / orig_w)
    # w = sqrt(max_pixels * orig_w / orig_h)
...
def main():
    ...
    if use_scale:
        pass1 = f'ffmpeg -y -i "{input_file}" -vf {scale_filter} -c:v libx265 -b:v {video_kbps_int}k -x265-params pass=1 -an -f null NUL'
        pass2 = f'ffmpeg -i "{input_file}" -vf {scale_filter} -c:v libx265 -b:v {video_kbps_int}k -x265-params pass=2 -c:a aac -b:a {audio_kbps}k "{output_file}"'
    else:
        pass1 = f'ffmpeg -y -i "{input_file}" -c:v libx265 -b:v {video_kbps_int}k -x265-params pass=1 -an -f null NUL'
        pass2 = f'ffmpeg -i "{input_file}" -c:v libx265 -b:v {video_kbps_int}k -x265-params pass=2 -c:a aac -b:a {audio_kbps}k "{output_file}"'
